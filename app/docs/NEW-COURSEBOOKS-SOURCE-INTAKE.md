...
The metadata-only `content/book-recordings.json` registers the 82 verified local MP3s. A lesson material can reference `/book-recordings/clear-speech-3-track-001.mp3` through `/book-recordings/clear-speech-3-track-082.mp3`. Each entry preserves the book, unit, exact task letter, source SHA-256 and duration. `GET` and `HEAD` support HTTP byte ranges and ETags for audio playback and seeking. The server first opens the registered ID beneath private `app/data/book-recordings`, then falls back to its registered source path beneath `книги` only when the private copy is absent. Both roots use `os.OpenRoot`. Exact response bytes are verified against SHA-256; an altered private copy returns HTTP 409 even when the original remains valid. Missing recordings return 404. No source recording is distributed in Git.

Run `python app/scripts/cache_book_recordings.py` from the repository root to copy all 82 registered recordings into private app data. It checks every source and existing destination first, preserves original files, and refuses to replace a different existing copy. Copy `app/data/book-recordings` separately when moving computers; then playback does not depend on the original books folder. Source-based re-authoring still needs its original private inputs.
