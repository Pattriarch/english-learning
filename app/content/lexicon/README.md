...
User-list extension, 2026-09-11: `coca-extension.json` adds 763 prepared US-English articles to the unchanged 10,188-entry base (10,951 words total, plus 36 separate phrases). The uploaded WSL has 10,000 unique surface tokens: 5,234 exact existing headwords, 3,020 form/variant links, 763 new entries, and 983 explicit reference-only items. `coca-import-audit.json` accounts for every source row. The filename does not establish an official COCA rank or CEFR level. See `app/docs/COCA-UPLOAD-2026-09-11.md`. Rebuilding the base reserves extension headwords to preserve separate ownership and stable IDs.
