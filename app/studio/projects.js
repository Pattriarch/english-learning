...
export async function mountProjects(root,data,refresh,id=''){
 ...
 function drawUnit(){
  ...
  const dictate=$('#project-dictate',root);if(dictate)dictate.onclick=event=>{
   const check=$('#project-check',root);if(!own()||check.disabled&&!event.currentTarget.classList.contains('recording'))return;
   return voice(event.currentTarget,answer,data.settings,()=>{if(own()&&answer.isConnected)answer.oninput();},{check,preview:$('#project-dictation-preview',root)});
  };
