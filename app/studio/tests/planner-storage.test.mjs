...
test('Legacy book metadata is fetched only for started source practice, regardless of current level',()=>{
 ...
 const ids=plannerBookRequests(data,'B1');assert.deepEqual(ids,[units[18].id,units[19].id]);assert.deepEqual(plannerBookRequests(data,'C2'),ids);
 data.state.attempts=[];assert.deepEqual(plannerBookRequests(data,'B1'),[]);
});
...
test('Book metadata loading limits concurrency, checks provenance, and keeps only exercise metadata',async t=>{
 ...
 const data={library:{books:[{id:'grammar-intermediate',level:'B1–B2',units}]},state:{attempts:units.slice(0,8).map(unit=>({lessonId:'book-'+unit.id,answer:'My independent response.'}))},bookStatus:{units:Object.fromEntries(units.map(unit=>[unit.id,{status:'ready'}]))}};
 ...
 const lessons=await loadPlannerBookLessons(data,'B1');assert.equal(peak,4);assert.equal(lessons.length,7);assert.equal(lessons.some(lesson=>lesson.id.endsWith('002')),false);assert.deepEqual(lessons[0].exercises,[{id:await bookExerciseID({id:'e1',kind:'write',prompt:'Long prompt'}),kind:'write'}]);assert.equal(lessons[0].sections,undefined);
});
