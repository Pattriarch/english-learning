...
export function mountPractice(root,data,mode,refresh,planned=null){
 ...
 function draw(){
  ...
  $('#practice-work').innerHTML=`<div class="spread" style="margin-bottom:22px"><span class="eyebrow" style="margin:0">${esc(task[0])}</span><button class="btn small" id="new-task">${icon('loop')} Другое задание</button></div>
   ...
   <div class="field"><label for="answer">${mode==='translation'?'Передай мысль на английском':'Твой ответ на английском'}</label><textarea id="answer" class="answer-area" rows="${mode==='writing'?10:7}" spellcheck="false" placeholder="${mode==='speaking'?'Нажми «Ответить голосом» или начни писать…':'Начни с собственной мысли…'}">${esc(getDraft(key,data.state))}</textarea></div>
   <div class="answer-meta"><span id="word-count"></span><span>Ctrl + Enter — проверить</span></div>
   <div class="exercise-actions"><div class="actions"><button id="voice" class="btn">${icon('mic')} Ответить голосом</button><button id="read-answer" class="btn ghost small" aria-label="Прослушать свой текст">${icon('sound')}</button></div><button id="check" class="btn primary">Получить разбор ${icon('arrow')}</button></div>
...
export function mountSettings(root,data,refresh){
 ...
 <section class="card"><h2>Голос и Anki</h2><p>Для локального голоса можно запустить <a href="https://github.com/ggml-org/whisper.cpp/tree/master/examples/server" target="_blank" rel="noreferrer" style="color:var(--blue)">whisper.cpp server</a> с английской моделью и вписать адрес /inference. В приложение звук отправляется как WAV 16 кГц.</p><p>Для прямой передачи карточек открой Anki с дополнением AnkiConnect: <code>2055492159</code>. Можно обойтись без дополнения: скачать ZIP с TSV и картинками в «Повторении».</p></section>
 </div></div>`;
