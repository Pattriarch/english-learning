...
export async function mountBookUnit(root,data,id,refresh){
 ...
 let tab='theory',step=Math.max(0,lesson.exercises.findIndex(ex=>initialStudy?ex.id===initialStudy.nextExerciseId:!answered.has(ex.id))),playRequest=0,transferTimer;
 ...
 function drawPractice(main){
  ...
  let request=uid(),inputMode='writing';
  main.innerHTML=`<section class="surface book-practice"><div class="book-exercise-nav" role="group" aria-label="Задания">${lesson.exercises.map((e,i)=>{if(stage&&!stage.exerciseIds.includes(e.id))return '';const a=bookAttemptState(data.state,lesson,data.library).attempts.filter(a=>a.lessonId===lesson.id&&a.exerciseId===e.id).at(-1),done=study?study.acceptedExerciseIds.includes(e.id):a?.feedback?.verdict==='correct';return `<button data-book-exercise="${i}" class="${i===step?'active':done?'done':''}" aria-label="Задание ${i+1}" aria-pressed="${i===step}">${i+1}</button>`;}).join('')}</div>${stage?`<div class="book-current-stage"><span class="eyebrow">${esc(stage.title)}</span><p>${esc(stage.purpose)}</p></div>`:''}<div class="eyebrow">${esc(kindName[ex.kind]||'Сформулировать самостоятельно')} · ${step+1} / ${lesson.exercises.length}</div><h2 class="book-prompt">${esc(ex.prompt)}</h2><section class="lesson-materials book-task-materials" id="book-task-materials" aria-label="Материалы текущего задания" hidden></section>${ex.context?`<div class="book-task-context">${bookParagraphs(ex.context)}</div>`:''}${productionHTML}${transferLocked?`<div class="book-transfer-lock" role="status"><strong>${study.dueAt===null?'Сначала заверши исправления':'Вернись к этому заданию позже'}</strong><p>${study.dueAt===null?'Перенос откроется через '+lesson.studyPlan.transfer.delayDays+' дней после верного разбора всех исправлений. Задание можно посмотреть заранее и сохранить черновик.':'Проверка будет доступна с '+esc(dueDate(study.dueAt))+'. Пауза нужна, чтобы извлечь знания из памяти и применить их в новой ситуации.'}</p></div>`:''}<label class="field-label" for="answer">Твой ответ на английском</label><textarea id="answer" class="answer-area" rows="9" placeholder="Напиши целый ответ. Допустимые варианты тоже учитываются."></textarea><div class="answer-meta"><span id="book-words"></span><span>Черновик сохраняется</span></div><div class="actions"><button class="btn" id="book-voice">${icon('mic')} Ответить голосом</button><button class="btn primary" id="book-check" ${transferLocked?'disabled':''}>Получить разбор ${icon('arrow')}</button></div><audio id="audio-preview" class="audio-preview" controls hidden></audio><p class="small-note book-voice-note">При диктовке проверяется расшифровка: смысл, грамматика и словоупотребление. Произношение по тексту не оценивается.</p>${ex.hint?`<details class="book-hint"><summary>Небольшая подсказка</summary>${bookParagraphs(ex.hint)}</details>`:''}<div id="book-feedback"></div><details class="book-reference"><summary>Разобрать пример ответа</summary>${ex.answers.map(a=>`<p class="book-en">${esc(a)}</p>`).join('')}${bookParagraphs(ex.explanation)}</details><div class="book-bottom-actions"><button class="btn small" id="book-ex-prev" ${step===0?'disabled':''}>← Предыдущее</button><button class="btn small" id="book-ex-next" ${step===lesson.exercises.length-1?'disabled':''}>Следующее →</button></div>${practiceFooter}</section>`;
  ...
  try{const input=JSON.parse(getDraft(inputKey,data.state));if(input?.version===1&&input.answer===target.value)inputMode=input.mode==='speaking'?'speaking':'writing';}catch{}
  const onInput=()=>{if(!current()||!target.isConnected)return;queueDraft(key,target.value);queueDraft(inputKey,JSON.stringify({version:1,answer:target.value,mode:inputMode}));request=uid();counter.textContent=words(target.value)+' слов';feedback.innerHTML='';};
