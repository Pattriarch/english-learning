import {bookPreparationHTML,bindBookPreparation} from './book-preparation.js';
...
import {lessonGuidanceHTML} from './lesson-guidance.js';
...
export async function bookExerciseID(exercise,materials=[]){
 ...
 if(exercise.guidance){const g=exercise.guidance;text+='\u001fguidance-v1\u001f'+[exercise.practiceStage||'',g.title||'',g.body||'',g.example||'',g.translation||''].join('\u001e');}
...
export async function mountBookUnit(root,data,id,refresh){
 ...
 function drawPractice(main){
  ...
  const check=$('#book-check',main);check.onclick=e=>busy(e.currentTarget,async()=>{
   ...
   if(!current())return;
   updateProgress();if(target.isConnected&&$('#answer',root)===target&&target.value===answer&&request===payload.id){feedback.innerHTML=feedbackHTML(result);bindMistakes(main);}else toast('Разбор предыдущей версии сохранён в журнале.');
  },'Разбираем ответ…');
  ...
  $('#book-ex-prev',main).onclick=()=>{step=exerciseOrder[position-1];draw();};$('#book-ex-next',main).onclick=()=>{step=exerciseOrder[position+1];draw();};bindMistakes(main);
 }
 $$('[data-book-tab]',root).forEach(b=>b.onclick=()=>{tab=b.dataset.bookTab;draw();});
 $$('[data-book-section]',root).forEach(b=>b.onclick=()=>{tab='theory';draw();const section=$('#book-section-'+b.dataset.bookSection,root);section.open=true;section.scrollIntoView({block:'start',behavior:'smooth'});});draw();
}
