...
export function materialHTML(material,index,state,lessonId){
 const audio=originalMaterialAudioURL(material),authentic=!!audio||material.inputSkill==='listening',playable=material.kind!=='reference'&&!authentic,listening=material.kind==='listening'||material.kind==='dialogue',picture=material.figure?.format==='png'&&Boolean(materialFigureHTML(material.figure,material.kind)),reference=material.kind==='reference',textHidden=listening||picture||reference,key=`material:notes:${lessonId}:${material.id}`;
...
export function mountLessonMaterials(root,lesson,exercise,state){
 ...
 $$('[data-material-toggle]',root).forEach(button=>button.onclick=()=>{const text=$(`[data-material-text="${button.dataset.materialToggle}"]`,root),m=materials[+button.dataset.materialToggle];text.hidden=!text.hidden;button.textContent=text.hidden?(m.figure?.format==='png'?'Открыть текстовое описание':m.kind==='reference'?'Открыть справку':m.kind==='reading'?'Открыть текст':'Открыть расшифровку'):'Скрыть текст';button.setAttribute('aria-expanded',String(!text.hidden));const note=$(`[data-material-reference-status="${button.dataset.materialToggle}"]`,root);if(note&&!text.hidden)note.hidden=false;});
