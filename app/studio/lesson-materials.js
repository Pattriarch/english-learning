...
export function materialSourceHTML(material){
 ...
 return `<a class="btn small" href="${esc(url.href)}" target="_blank" rel="noopener noreferrer">Открыть запись и расшифровку ${icon('arrow')}</a>`;
}
export function materialHTML(material,index,state,lessonId){
 const authentic=material.inputSkill==='listening',playable=material.kind!=='reference',listening=material.kind==='listening'||material.kind==='dialogue',picture=material.figure?.format==='png'&&Boolean(materialFigureHTML(material.figure,material.kind)),textHidden=listening||picture,key=`material:notes:${lessonId}:${material.id}`;
...
export function mountLessonMaterials(root,lesson,exercise,state){
 ...
 $$('[data-natural-audio]',root).forEach(player=>{let restarting=false;player.onplay=async()=>{if(restarting)return;restarting=true;const current=beginAudio();try{await player.play();if(!current())player.pause();}catch{}finally{restarting=false;}};});
