...
export async function mountDailyPlanner(root,data,refresh){
 ...
 function draw(){
  ...
  const challenge=plan?.beginner?{}:weeklyChallenge(challenges,data.state,preferences.level,new Date());
  const actualMinutes=Math.floor(Math.max(0,Number(data.state.activity?.[selected])||0)/60),dates=weekly.days||[],transfers=historical||plan?.beginner?null:transferQueue(data,new Date());
  const courses=historical||plan?.beginner?null:coursebookQueue(data,new Date(),3),courseSteps=courses?[...courses.due,...courses.ready].slice(0,3):[];
  const optionalBooks=historical||plan?.beginner?[]:optionalBookPractice(data,3);
  ...
  const canRestart=!historical&&!plan?.beginner&&['A1','A2'].includes(plan?.level)&&data.lessons.some(l=>l.beginner&&l.level===plan.level);
  root.innerHTML=`<div class="daily-planner">${canRestart?'<section class="card"><h2>Начальный курс теперь идёт по шагам</h2><p>Сначала короткое объяснение и одна фраза. Можно заменить сегодняшний прежний план новым; старый план и все ответы сохранятся в истории.</p><button class="btn primary" id="beginner-plan-refresh">Пересобрать сегодняшний план</button></section>':''}
   ...
   <div class="planner-layout"><section class="planner-agenda" aria-labelledby="planner-agenda-title"><div class="planner-section-heading"><div><span class="planner-kicker">${historical?'ПЛАН ИЗ ИСТОРИИ':'ТВОЙ МАРШРУТ'}</span><h2 id="planner-agenda-title">${historical?'Задания этого дня':'Занятия по порядку'}</h2></div><span>${plan?plan.blocks.length+' блоков':'План не сохранён'}</span></div>
    ...
    ${challenge.unit?`<section class="planner-followup"><span class="planner-kicker">ПРОЕКТ НЕДЕЛИ · ${esc(challenge.unit.level)}</span><h3>${esc(challenge.unit.title)}</h3><p>${esc(challenge.title)}. Выдели отдельные 15–25 минут на этот шаг или продолжи в другой день.</p><p class="small-note">${challenge.progress.attempted} / ${challenge.progress.total} основных ответов · затем доработка и новая ситуация через 7 дней.</p><a class="planner-link" href="${esc(challenge.href)}">${challenge.progress.due?'Пора применить снова':'Открыть текущий шаг'} ${icon('arrow')}</a></section>`:challenge.waiting?`<section class="planner-followup"><span class="planner-kicker">ПЕРЕНОС ПРОЕКТА</span><h3>Вернуться к новой ситуации</h3><p>${esc(challenge.waiting.unit.title)} · с ${new Date(challenge.waiting.progress.dueAt).toLocaleDateString('ru-RU')}.</p><a class="planner-link" href="#/projects">Все проекты ${icon('arrow')}</a></section>`:challenges&&!plan?.beginner?'<section class="planner-followup"><h3>Проекты этого уровня пройдены</h3><p>Посмотри разборы, повтори трудные задачи или выбери следующий уровень программы.</p><a class="planner-link" href="#/mastery">Карта результатов →</a></section>':''}
    ${!plan?.beginner?`<section class="planner-followup planner-cinema"><span class="planner-kicker">${icon('play')} АНГЛИЙСКИЙ ЧЕРЕЗ ИСТОРИИ</span><h3>Лучше звоните Солу</h3><p>Одна сцена, несколько живых выражений и твой пересказ. В киноклубе есть план работы с эпизодами.</p><a class="planner-link" href="#/cinema">Открыть киноклуб ${icon('arrow')}</a></section>`:''}
   </aside></div>
  ...
  const restart=$('#beginner-plan-refresh',root);if(restart)restart.onclick=()=>busy(restart,async()=>{
   if(!current()||planWriteBusy)return;planWriteBusy=true;try{
    const snapshot=todayPlan,archiveKey='planner:archive:'+today+':'+Date.now();
    const replacement=createDailyPlan(data,{level:snapshot.level,minutes:snapshot.minutes,domain:snapshot.domain},new Date());
    if(!validDailyPlan(replacement,today)||!replacement.beginner)throw Error('Начальные уроки уже пройдены. Продолжай текущий план.');
    await saveConfirmed(archiveKey,snapshot);if(!current()||dateKey()!==today)return;
    await saveConfirmed(dayKey(today),replacement);todayPlan=replacement;
