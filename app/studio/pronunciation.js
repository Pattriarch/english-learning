...
function mountLesson(root,data,catalog,l,refresh){
  ...
  <section class="surface"><span class="eyebrow">ПРИМЕНИ И ОБЪЯСНИ</span><p class="sound-task">${esc(l.practice.prompt)}</p><div class="answer-input-header"><label class="field-label" for="sound-answer">Твой ответ и наблюдения</label><button type="button" class="btn" id="sound-dictate" aria-controls="sound-answer">${icon('mic')} Надиктовать ответ</button></div><textarea class="answer-area" id="sound-answer" rows="7" placeholder="Выполни задание и напиши, что услышал или изменил в своём чтении…">${esc(saved)}</textarea><p class="small-note">Можно напечатать ответ или наговорить его по-английски — расшифровка появится в этом поле.</p><div class="actions"><button class="btn primary" id="sound-check">Разобрать мой ответ ${icon('arrow')}</button></div><audio id="sound-dictation-preview" class="audio-preview" controls hidden></audio><div id="sound-feedback">${attempt&&saved.trim()===attempt.answer?feedbackHTML(attempt):''}</div><p class="small-note sound-check-note">Разбор относится к тексту ответа. Распознавание слов не показывает точность звуков: сравнивай произношение самостоятельно с записью и образцом.</p><details class="sound-reference"><summary>Свериться с примером после своей попытки</summary><p>${esc(l.practice.reference)}</p></details>
  ...
  $('#sound-dictate',root).onclick=ev=>{if(target.readOnly)return;return voice(ev.currentTarget,target,data.settings||{},()=>target.oninput(),{check:$('#sound-check',root),preview:$('#sound-dictation-preview',root)});};
  ...
  $('#sound-check',root).onclick=ev=>busy(ev.currentTarget,async()=>{
    if(target.value.trim().length<2)throw Error('Сначала напиши или надиктуй свой ответ.');
  ...
  complete.onclick=async ev=>{await busy(ev.currentTarget,async()=>{
    ...
    stopAudio();const submitted=target.value,criteria=JSON.stringify([...checked]),inputs=$$('[data-criterion]',root),readOnly=target.readOnly;
