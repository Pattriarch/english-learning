...
import {speak,speechVoice,recordOnly,stopAudio,beginAudio} from './audio.js';
...
let samplesPromise;
const audioSamples=()=>samplesPromise||(samplesPromise=fetch('/assets/pronunciation-audio/index.json').then(r=>r.ok?r.json():null).catch(()=>null).then(value=>{if(!value)samplesPromise=null;return value;}));
...
function mountOverview(root,data,catalog){
  ...
  <details class="surface sound-sources"><summary>Транскрипция, образцы и самостоятельная проверка</summary><p>${esc(pronunciationModelLabel(catalog))} Озвучивание зависит от английских голосов твоей системы; выбранный голос показан внутри занятия. Для проверки конкретного звука доступны словарные образцы. Помощник разбирает написанное объяснение, а качество звуков и интонации ты проверяешь по записи.</p>${sourceLinks(catalog.sources)}</details>`;
}
...
function mountLesson(root,data,catalog,l,refresh){
  ...
  ${l.contrastPairs.length?`<section class="surface"><span class="eyebrow">СРАВНИ ЗВУЧАНИЕ</span><h2>Сначала услышать разницу.</h2><p class="small-note">Слова можно послушать. Схемы ударения и интонации показывают, что попробовать самому: системный голос не воспроизводит все обозначенные различия.</p>${l.contrastPairs.map(p=>`<div class="sound-pair"><div class="sound-pair-options">${pairOption(p.left,p.leftAudio)}${pairOption(p.right,p.rightAudio)}</div><p>${esc(p.note)}</p></div>`).join('')}</section>`:''}
  ...
  const target=$('#sound-answer',root),accent=$('#sound-accent',root),rate=$('#sound-rate',root),complete=$('#sound-complete',root);let requestID=uid(),samples=null,samplesLoaded=false,playRequest=0;
  const voiceInfo=()=>{const info=$('#sound-voice-info',root);if(!accent.isConnected||!info?.isConnected)return;const voice=speechVoice(accent.value),name=voice?.name||samples?.voice,lang=voice?.lang||samples?.culture;info.textContent=name?`${voice?'Голос браузера':'Локальные аудиообразцы'}: ${name} · ${lang}.${lang.replace(/_/g,'-').toLowerCase()!==accent.value.toLowerCase()?' Выбранный вариант недоступен: используется другой английский голос.':''} ${pronunciationModelLabel(catalog)}`:samplesLoaded?'Английский голос и локальные образцы недоступны. Используй словарные записи или открой приложение в Chrome/Edge.':`Загружаем английские аудиообразцы. Для проверки также доступны словарные записи. ${pronunciationModelLabel(catalog)}`;};
  audioSamples().then(value=>{samples=value;samplesLoaded=true;voiceInfo();});
  const onVoices=()=>{if(!root.isConnected||!accent.isConnected)window.speechSynthesis?.removeEventListener('voiceschanged',onVoices);else voiceInfo();};window.speechSynthesis?.addEventListener('voiceschanged',onVoices,{once:true});voiceInfo();accent.onchange=()=>{playRequest++;stopAudio();voiceInfo();};
  $$('[data-listen]',root).forEach(b=>b.onclick=async()=>{
    const audioCurrent=beginAudio(),text=texts[+b.dataset.listen],request=++playRequest;
    if(speechVoice(accent.value)){speak(text,+rate.value,accent.value);voiceInfo();return;}
    samples=await audioSamples();if(!audioCurrent()||!b.isConnected||request!==playRequest)return;const filename=samples?.clips?.[text];
    if(filename&&/^[a-f0-9]{64}\.wav$/.test(filename)){const player=$('#sound-model',root);player.src='/assets/pronunciation-audio/'+filename;player.playbackRate=+rate.value;player.hidden=false;try{await player.play();}catch{if(audioCurrent()&&b.isConnected)toast('Нажми ▶ в плеере образца, чтобы начать воспроизведение.');}}
    else toast('Для этого примера аудио недоступно. Открой словарный образец или используй английский голос Chrome/Edge.',true);
    voiceInfo();
